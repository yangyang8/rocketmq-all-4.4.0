...
/**
 * 集群状态的存储结构
 * 这个类主要有5个变量 ，集群的状态就保存在这五个变量中
 *
 *
 * 从上面这五个变 的定义，可以清楚地 出各个组件的状态是如何存储的，
 * NameServer 的主要工作就是维护这五个变量中存储的信息
 */
public class RouteInfoManager {
    private static final InternalLogger log = InternalLoggerFactory.getLogger(LoggerName.NAMESRV_LOGGER_NAME);

    //Broker超过2分钟没有更新时间，则认为Broker已经死了
    private final static long BROKER_CHANNEL_EXPIRED_TIME = 1000 * 60 * 2;
    //悲观锁
    private final ReadWriteLock lock = new ReentrantReadWriteLock();
    /**
     * topicQueueTable 这个结构的 Key 是topic 名称，它存储了所有 Topic
     ...
     * Key是BrokerAddr ，也就是对应着一台机器， BrokerAddrTable 中的 Key
     * 是BrokerName ，多个机器的 BrokerName 可以相同 BrokerLiveTable
     * 存储的内容是这台 Broker 机器的实时状态，包括上次更新状态的时间
     * 戳， NameServer 会定期检查这个时间戳，超时没有更新就认为这个
     * Broker 失效了，将其从 Broker 表里清除
     */
    private final HashMap<String/* brokerAddr */, BrokerLiveInfo> brokerLiveTable;


    /**
     * Filter Server 是过滤服务器，是 RocketMQ 的一种服务端过滤方式，一
     * 个Broker 可以有一个 多个 Filter Server 这个结构的 Key是Broker
