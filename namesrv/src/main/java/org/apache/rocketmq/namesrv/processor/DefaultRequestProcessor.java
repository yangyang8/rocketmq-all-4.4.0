...
/**
 * 因为其他角色会主动向Name Server 上报状态，所以 NameServer 的主要逻辑在 DefaultRequestProcessor 类中，
...
public class DefaultRequestProcessor implements NettyRequestProcessor {
    ...
    /**
     * NameServer 的核心业务逻辑，是在DefaultRequestProcessor.java中可以一目
     * 了然地看出，网络通信服务模块收到请求后，就调用这个Processor#processRequest来处理相关的请求，
     * @param ctx
     * @param request
     * @return
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx,
        ...
        /**
         * 辑主体是个switch语句，根据RequestCode调用不同的函数来处理。
         * 从requestCode中可以了解到 NameServer的主要功能，比 REGISTER_BROKER
         * 是在集群中新加入一个 Broker机器； GET_ROUTEINTO_BY_TOPIC
         * 是请求获取一个Topic的路由信息;WIPE_WRITE_PERM_OF_BROKER 是删除一个Broker的写权限
         */
        switch (request.getCode()) {
            case RequestCode.PUT_KV_CONFIG://
                return this.putKVConfig(ctx, request);
            case RequestCode.GET_KV_CONFIG://获取配置
                return this.getKVConfig(ctx, request);
            case RequestCode.DELETE_KV_CONFIG:
                return this.deleteKVConfig(ctx, request);
            case RequestCode.QUERY_DATA_VERSION://队列数据的版本
